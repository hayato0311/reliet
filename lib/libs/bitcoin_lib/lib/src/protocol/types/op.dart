...
enum Op {
  ...
  opPushBytes0._internal(0x00),

  // Push the next byte as an array onto the stack.
  opPushBytes1._internal(0x01),

  // Push the next 2 bytes as an array onto the stack.
  opPushBytes2._internal(0x01),
  ...
  factory Op.deserialize(Uint8List bytes) {
...
Expected: ${bytesLength()}, Actual: ${bytes.length}''');
    ...
    if (code < 0 || code > 255) {
      throw ArgumentError('"$code" is undefined Op code');
    }

    return Op(code);
  }

  const Op._internal(this.code);
